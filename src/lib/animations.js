...
export const animateFadeInUp = (targets, delay = 0) => {
  if (prefersReducedMotion()) return utils.set(targets, { opacity: 1, translateY: 0 });
...
export const animateStaggerReveal = (targets, staggerDelay = 100) => {
  if (prefersReducedMotion()) return utils.set(targets, { opacity: 1, scale: 1 });
  return animate(targets, {
    opacity: [0, 1],
    scale: [0.95, 1],
    translateY: [20, 0],
...
export const useMagneticEffect = (strength = 0.3) => {
  ...
  useEffect(() => {
    ...
    const move = (e) => {
      ...
      animate(el, {translateX: x * strength,
...
export const pageTransitionIn = () => {
  ...
  return animate("main", {opacity: [0, 1],
    translateY: [20, 0],
    duration: 600,
    easing: "easeOutExpo"});
};
